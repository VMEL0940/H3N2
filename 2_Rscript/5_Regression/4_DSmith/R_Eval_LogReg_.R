...
## Data import ##
NewHA_NS <- read.csv("Evaluation/HA242_NSdist.csv", header = T, na.strings = "")

colnames(NewHA_NS)[1] <- "ID"
NewHA_NS$ID <- as.factor(NewHA_NS$ID)

NewHA_NS_reg <- NewHA_NS %>% 
  filter(Group != "G17")

NewHA_NS_reg$Dominance_F <- factor(NewHA_NS_reg$Dominance2, levels = c(0, 1), labels = c("Extinct", "Dominant"))
...
Full_ME_model <- glmer(Dominance2 ~ Nmedian + (1 | Group), 
...
jpeg(filename = "/01_GenDisFlu/Fig/PredModel/ROC_Alldata.jpeg", width = 180, height = 160, units = "mm", res = 520)
...
plot(roc_score, col="black",  
     ...
     main ="1. ROC curve of the Dominance Prediction Model - 16 group")
...
jpeg(filename = "/01_GenDisFlu/Fig/PredModel/ROC_BALdata.jpeg", width = 180, height = 160, units = "mm", res = 520)
#ROC-curve using pROC library
