...
trees <- read.mcmctree(file = treeFiles[currentSegment.treeFile.index])  ## Take long time to bring substree files 
nrLookedAtTrees <- length(trees)

# collect tip info in list of data frames 
vaccinePeriods <- 1:2  ## (3) Number of vaccine period
##########
# for HA #
##########
inFileNames <- c("9_NS_HK15_195seq.fasta","10_NS_Kan17_18seq.fas")   ## (4) List of Fasta Files 
vaccineStrains <- c("EPI686117_A_Hong_Kong_15611_2015",
                    "MG974447_A_Kansas_14_2017")  ## (5) List of Vaccine strains 
...
####################################
for (current.vaccinePeriod in 1:2){  ## (6) Number of vaccine period
